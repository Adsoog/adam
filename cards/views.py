...
from .models import Cards, Tasks, CardsTasksOrder
...
class DailyCardList(ListView):
    ...
    def get_queryset(self):
        ...
        return Cards.objects.filter(user=user, id=card_id)
    ...
    def get_context_data(self, **kwargs):
        ...
        if card:
            tasks_ordered = CardsTasksOrder.objects.filter(card=card).order_by('order').select_related('task')
...
def add_daily_task(request):
    ...
    card = get_object_or_404(Cards, id=card_id, user=user)

    daily_task = Tasks.objects.create(
        verb=verb,
        object=object_,
    ...
    CardsTasksOrder.objects.create(task=daily_task, card=card, order=max_order)
...
def delete_daily_task(request, pk):
    ...
    task_order = get_object_or_404(CardsTasksOrder, pk=pk)
...
def search_task(request):
    ...
    results = Tasks.objects.filter(user=request.user, verb__icontains=search_text)
...
def add_task_to_card(request, task_id):
    ...
    card = get_object_or_404(Cards, id=card_id, user=request.user)
    task = get_object_or_404(Tasks, id=task_id, user=request.user)
    
    # Verificar si la tarea ya está asociada a la tarjeta
    if CardsTasksOrder.objects.filter(card=card, task=task).exists():
    ...
    max_order = get_max_order(card)
    CardsTasksOrder.objects.create(card=card, task=task, order=max_order)

    # Obtener las tareas ordenadas
    tasks_ordered = CardsTasksOrder.objects.filter(card=card).order_by('order').select_related('task')
    daily_tasks = tasks_ordered  # Aquí no extraemos los tasks, sino que mantenemos los objetos CardsTasksOrder
...
def sort_tasks(request):
    ...
    for idx, task_pk in enumerate(task_pks_order, start=1):
        task_order = get_object_or_404(CardsTasksOrder, pk=task_pk, card_id=card_id)
...
def toggle_task_state(request, pk):
    task_order = get_object_or_404(CardsTasksOrder, pk=pk, card__user=request.user)
