...
from cards.utils import get_max_order
from cards.utils import CardsTasksOrder
...
def replicate_tasks_to_new_card(source_card, target_card):
    ...
    with transaction.atomic():
        tasks_to_replicate = CardsTasksOrder.objects.filter(card=source_card)
        for task_order in tasks_to_replicate:
            # Verificar si la tarea ya existe en la target_card
            if not CardsTasksOrder.objects.filter(card=target_card, task=task_order.task).exists():
                CardsTasksOrder.objects.create(
                    card=target_card,
...
def replicate_last_week_tasks(request):
    ...
    last_week_cards = Cards.objects.filter(
        user=user,
    ...
    for last_week_card in last_week_cards:
        ...
        new_card, created = Cards.objects.get_or_create(
            user=user,
            date=new_card_date,
...
def replicate_tasks_to_future_week(request):
    ...
    while True:
        target_week_cards = Cards.objects.filter(
            user=user,
    ...
    this_week_cards = Cards.objects.filter(
        user=user,
    ...
    for this_week_card in this_week_cards:
        ...
        target_week_card, created = Cards.objects.get_or_create(
            user=user,
...
def replicate_tasks_to_previous_week_filling_gaps(request):
    ...
    for this_week_card in this_week_cards:
        ...
        if not days_with_tasks[target_card_date]:
            target_card, created = Cards.objects.get_or_create(
                user=user,
